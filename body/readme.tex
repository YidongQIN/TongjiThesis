...
\begin{quote}
以下称同济大学研究生院的《同济大学学位论文写作示例（2017）》与《同济大学研究生学位论文写作规范（试行）（2017）》为\texttt{}校方模版''；
称 marquistj13 的 TongjiThesis 模版为\texttt{}原模版''。
\end{quote}
...
\item
  各个元素之间的间距没有出确定值，只是大概的\texttt{}换一行''、\texttt{}空半行''。
\item
  封面内容的行距被 Word
  的\texttt{}对齐到网格''控制，导致展示的样式与本模板有偏差。

  比如申请人个人信息表格的行距明显不是单倍，根据目测调整为 1.75 倍。
\item
  申请人个人信息表格要求\texttt{}左侧缩进 4.5
...
\item
  ...
  \begin{itemize}
  \item
    主要处于代码美观简介考虑，因为\texttt{}姓名''、\texttt{}学号''和\texttt{}指导教师''等文字并不会复用，所以没必要用变量替代。
  \item
...
\texttt{\textasciitilde{}}
连接，以避免不良的分行，打断了\texttt{}图''、\texttt{}表''、\texttt{}第''等前缀词、编号数字和可能的后缀\texttt{}节''、\texttt{}章''。

比如说，\texttt{见\textbackslash{}refsec\{\}。}
可以直接生成\texttt{}见第\textsubscript{X.X}节。''这样的语句。 而且，好处是 VS Code 依然识别这个命令是 \texttt{\textbackslash{}ref}，可以自动联想到 label。
...
\begin{quote}
以前 CAD 课的要求\texttt{}数字要比汉字小一号''，算是一个纪念画图日子的小彩蛋。
\end{quote}
...
\begin{itemize}
\item
  inline 是用 \texttt{}(1)''进行编号的。
\item
  inlinecn 是用\texttt{}第一''进行编号的。
...
使用，也可以把其中\texttt{}注释''类型的绿色改为深绿色，以求美观； 3.
...
学校的版面设置要求是\texttt{}上、下 2.54cm，左、右 3.17cm，页眉、页脚
...
为微积分中的 \(d\) 符号设定了\texttt{}左侧有间距、直立体''的命令
...
用 \texttt{headsep} 把页眉\texttt{}顶''上去。 \(headsep = 0.7*20bp = 4.94mm\)。
...
但是如上处理，发现页眉相关尺寸参数比较\texttt{}奇怪''，可能回给后续修改者带来困惑。
...
校方模板中，这个\texttt{}紧凑''的边距并不明显，是因为最后一行如果行距不够的情况，会排入下一页；
从而相当于空出了将近一行的距离，正文底线到页底的边距可能超过
\(25.4mm + 20bp = 3cm\)；
换句话说，如果全用正文字体多填充几页，就很容易发现这个边距其实挺小的。

页眉部分同理，在 word 中控制\texttt{}页面上边距 2.54cm，页眉
2.0cm''并且多用正文填充几页，也可以发现，页眉、分割线和正文会挤在一起。

校方的\texttt{}模板''有的细节经不起推敲，自己取舍吧。
...
\item
  ...
  \end{itemize}
\item
  如果放弃\texttt{}黑体''，可以选择\texttt{}雅黑''等针对中英文混排优化过的无衬线字体。
\item
...
中的\texttt{}第''字之后手动加一个六分之一小空格 \texttt{\textbackslash{},}
以规避 \texttt{xeCJK} 自作主张的小聪明。

\subsection{强迫症的补完}

\texttt{ctex} 提供的章节标题命令最小是
\texttt{\textbackslash{}subparagraph}、即5级标题，形如\texttt{}x.x.x.x.x.x''共6个数字。
...
|-- appendix\ % 附录
    |-- appendix.tex % 附录章节
        （可以在一个.tex文件中分 chapter，也可以每章一个.tex文件）
|-- body\ % 正文
    |-- chapter1.tex % 单独章节
    |-- chapter2.tex
...
\LaTeX 原生没有\texttt{}公式索引''这样的命令，所以是从清华的模板中学到的命令。
...
\item
  ...
  中文封面中，如果最上方添加了\texttt{}打印时删除''\texttt{}电子版''或者\texttt{}保密''的标签字样时候，原本的
...
\item
  ...
  中文封面的申请人信息表格，第一、二行\texttt{}姓名''、\texttt{}学号''因为字数太少，会有
  ...
  \begin{itemize}
  \item
    当然，如果是有副导师的情况，则\texttt{}所在院系''\texttt{}学科门类''等四字列仍然会有
...
好像可以加速编译，所以如果单章节的编译与总编译的时间相差不多，那么分章节的优势可能就仅在于\texttt{}方便分章节检查''这样的了------然而有同步、有电子书签的情况下，也没那么大优势。
