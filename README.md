...
因为自己的 $\LaTeX$ 知识水平太低，对主要宏包的运用可谓 naive，身边也没有人能传授一点 $\LaTeX$ 的经验，所以最终也只是做了一点微小的工作。
...
封面 `electronic` 和 `secret` 选项均已失去了意义；
但考虑到日后万一又加了回来，故不做删除、予以保留。

此外，尝试采用`CTeX` 提供的 `\zihao{}` 命令，但是发现反而没有原本的字号命令好用。
...
加上原创性声明和授权书页面生成，现在共有三个新定义的 make 命令。
* `\makecover{../pages/cover} `
* `\makeauthorizationpage[../pages/scantmpauth.pdf] %可导入扫描页`
* `\makedeclarepage%[../pages/scantmpauth.pdf] 不导入扫描页则生成电子版`

它们在 `frontmatter` 之前，其页面布局由 `\AtBeginDocument{}` 中的 `\pagestyle{tongji@empty}` 和 `\pagenumbering{roman}` 控制，即在 PDF 中采用**小写罗马字母**页码（文件中不会打印编号，只有电子书签）。
...
以页眉部分设置为例。
如果排版效果跟校方模版的样本（而不是跟其描述内容）一致的话，需要如下设置。

1. `top=29.63mm`

   学校的 2.54cm 是页眉那条横线到页面上边缘的距离，所以第一行文字到页面上边缘的距离是略大于 2.54cm 的。

   计算输入 `\geometry={}` 的 `top=` 应该加上一个行间距减去字高，约 $2.54cm + (20-12)bp = 25.4mm + 12*(25.4mm/72) = 29.63mm$ 。

   > $1 inch = 25.4 cm$ 是 word 的默认值。

2. `headsep=4.23mm`

   而如果按照原 $\LaTeX$ 模板的设置，`top=25.4mm`、`headsep=5.4mm`，那么页眉那条横线的位置实际上是 $top-headsep=20mm$。

   所以，`top=29.63mm` 对应的 `headsep` 应约为 $29.63mm - 25.4mm = 4.23mm$。

3. `headheight=23.7mm`

   而 `headheight` 计算页眉行行底到页上边距，需要是加上页眉的行高，即约 $20mm + 10.5bp = 23.7 mm$。

4. `footskip=5.4mm`

   页脚的计算同理。`\footskip` 指的是 baseline of last line of text and baseline of footer 的距离。
   ...
   换句话说，如果全用正文字体多填充几页，就可以显示出这个边距其实挺小的。

在 word 中，手动控制“上边距 2.54cm，页眉 2.0cm”并且多用正文填充几页，也可以发现，页眉、分割线和正文会挤在一起。
